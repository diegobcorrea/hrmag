...
function do_insert() {	
	if( 'POST' == $_SERVER['REQUEST_METHOD'] 
		&& !empty( $_POST['action'] ) 
		&& $_POST['post_type'] == 'fichas' ) { // Check what the post type is here instead
		...
		$post = array(
			...
			'post_type'		=> 'fichas' // Set the post type based on the IF is post_type X
		);
...
<div id="postbox">
	...
	<form id="new_post" name="new_post" method="post" action="<?php bloginfo('template_directory');?>/post-process.php">
		...
		<input type="hidden" name="post_type" id="post_type" value="fichas" />
